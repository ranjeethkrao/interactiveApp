...
router.get('/emailExists', (req, res) => {
    ...
    // firebaseDB.ref().child('/USERS').orderByChild('email').equalTo(req.query.email).on("child_added", function(snapshot) {
    ...
    firebaseDB.ref('users').once("value", function(snapshot){
        Object.keys(snapshot.val()).forEach(key=>{
            if(snapshot.val()[key].email === req.query.email){ console.log('found!!!!!!!!!!!!!!!!!!')
            return res.send(true);}
        })
